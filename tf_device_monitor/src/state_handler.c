...
_Noreturn void client_app(void)
{
...
static error_code_t client_handler(void)
{
    ...
    switch (clientInfo.state)
    {
        ...
        {
            error_code_t errorCode = OK;
            syslog(LOG_DEBUG, "dm STATE SETTINGS");
...
static pass_id_e checkMD5Sum()
{
    ...
    char cmd_getUserConfig[] = "ubus call uci get '{\"config\":\"rpcd\"}'";
    userConfig = openPipe(cmd_getUserConfig);

    char hash_data[MD5_LEN];
    strcpy(hash_data, "");
    while (fgets(output, OUTPUT_MAX_LENGTH, userConfig))
    {
        printf("%s\n", output);
        if (strstr(output, "hash"))
        {
            printf("--- >>> hash found!\n");
            getData_formJson(output, "hash", hash_data);
            printf("hash_data = %s\n", hash_data);
        }
    }

    closePipe(userConfig, cmd_getUserConfig);
    printf("pass_md5 = %s\n", settingsMsg.settMsg.settings_struct.pass_md5);

    for (int i = 0; i < MD5_LEN; i++) {
        if (settingsMsg.settMsg.settings_struct.pass_md5[i] != hash_data[i]) {
