...
static void getConfigParam(cmd_type_e cmdType, char *param, char * logMgs)
{
    ...
    closelog();
    closePipe(pipe, (char *) CommandTable[cmdType]);
}
...
static void getMacaddr(search_out_msg_t *msg)
{
    ...
    for (int i = 0; i < MAC_LEN; i++)
    {
        ...
        printf("logMgs: %s %s %ld", logMsg, output, macBit);
...
int getTimeInfo(search_out_msg_t *searchOutMsg)
{
    ...
    closePipe(pipe, (char *)CommandTable[CMD_TIME_INFO]);
    return EXIT_SUCCESS;
}
...
int getBoardInfo(search_out_msg_t *searchOutMsg)
{
    ...
    closelog();
    closePipe(pipe, (char *)CommandTable[CMD_BOARD_INFO]);
...
int getSystemInfo(search_out_msg_t *searchOutMsg)
{
    ...
    closePipe(pipe, (char *)CommandTable[CMD_SYSTEM_INFO]);
    return 0;
}
